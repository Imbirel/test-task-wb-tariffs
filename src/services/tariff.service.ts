...
import { WbResponseSchema, type TariffRow } from "#types/tariff.js";
...
export const fetchAndSaveTariffs = async () => {
    try {
        ...
        const rawData = await getWBTariffs(WB_API_TOKEN);
        ...
        const { warehouseList, dtNextBox, dtTillMax } = result.data;
        const today = getTodayDate();

        const rows: TariffRow[] = warehouseList.map((item) => ({
            date: today,
        ...
        logger.error(`Ошибка при получении или сохранении тарифов: ${err.message}`);
    }
};
